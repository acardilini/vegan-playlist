...
2. **Shell layout — the 5 areas are a horizontal TOP BAR; the queue rail is the left column inside Songs.** _(Revised during A2 execution: the mockup's "Option B" put the 5 areas in a left sidebar, but the curator's actual preference is the area nav **above** — a top bar — with the queue-rail column beneath it on the left. `AdminLayout` renders a full-width `.admin-topbar`; `SongsArea` renders `.songs-layout` = rail column + list. Applies to A3/A4 too.)_
